<script lang="ts">
  ...
  $: teamsWithLocations = data.teamsWithLocations || []
</script>

<div class="p-4 md:p-8">
  <div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold">All Locations</h1>
    <!-- You can add a "Create Location" button here if desired -->
    <!-- <a href="/account/teams" class="btn btn-primary">Create Location</a> -->
  </div>
  ...
  {#if teamsWithLocations.length === 0}
    <div class="text-center py-12">
      ...
      <p class="mt-2 text-neutral-content">
        You need to be a member of a team to create locations.
      </p>
      <a href="/account/teams" class="btn btn-primary mt-4">Go to Teams</a>
    </div>
  {:else}
    <!-- Iterate over each team -->
    {#each teamsWithLocations as team}
      <div class="mb-10">
          ...
          {#each team.worlds as world}
            <div class="mb-6 pl-4">
              <div class="flex justify-between items-center mb-4">
                ...
                <a
                  href="/account/teams/{team.id}/worlds/{world.id}/elements/Location/create"
              ...
              {:else}
                ...
                >
                  <!-- Iterate over each location in the world and display it as a card -->
                  {#each world.elements as location}
                    <a
                      href={`/account/teams/${team.id}/worlds/${world.id}/elements/${location.id}`}
                    ...
                    >
                      <div class="card-body">
                        ...
                        <p class="text-sm text-neutral-content truncate">
                          {(typeof location.properties === "object" &&
                            location.properties !== null &&
                            !Array.isArray(location.properties) &&
