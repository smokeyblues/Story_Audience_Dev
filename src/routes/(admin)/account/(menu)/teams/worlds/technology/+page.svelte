<script lang="ts">
  ...
  $: teamsWithTechnology = data.teamsWithTechnology || []
</script>

<div class="p-4 md:p-8">
  <div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold">All Technology</h1>
  </div>

  {#if teamsWithTechnology.length === 0}
    <div class="text-center py-12">
      <p class="text-lg">No teams found.</p>
      <p class="mt-2 text-neutral-content">
        You need to be a member of a team to create technology.
      </p>
      <a href="/account/teams" class="btn btn-primary mt-4">Go to Teams</a>
    </div>
  {:else}
    <!-- Iterate over each team -->
    {#each teamsWithTechnology as team}
      <div class="mb-10">
          ...
          {#each team.worlds as world}
            <div class="mb-6 pl-4">
              ...
              {#if world.elements.length === 0}
                <div class="bg-base-100 rounded-lg p-6 text-center">
                  <p class="text-neutral-content mb-3">
                    No technology in this world yet.
                  </p>
                  <a
                    href="/account/teams/{team.id}/worlds/{world.id}/elements/Magic%20System%20%2F%20Technology/create"
                    class="btn btn-sm btn-primary"
              ...
              {:else}
                ...
                >
                  <!-- Iterate over each technology in the world and display it as a card -->
                  {#each world.elements as technology}
                    <a
                      href={`/account/teams/${team.id}/worlds/${world.id}/elements/${technology.id}`}
                    ...
                    >
                      <div class="card-body">
                        ...
                        <p class="text-sm text-neutral-content truncate">
                          {(typeof technology.properties === "object" &&
                            technology.properties !== null &&
                            !Array.isArray(technology.properties) &&
                            (technology.properties.description ||
                              technology.properties.rules)) ||
