<script lang="ts">
  ...
  $: teamsWithEvents = data.teamsWithEvents || []
</script>
  ...
  {#if teamsWithEvents.length === 0}
    <div class="text-center py-12">
      ...
      <p class="mt-2 text-neutral-content">
        You need to be a member of a team to create events.
      </p>
      <a href="/account/teams" class="btn btn-primary mt-4">Go to Teams</a>
    </div>
  {:else}
    <!-- Iterate over each team -->
    {#each teamsWithEvents as team}
      <div class="mb-10">
          ...
          {#each team.worlds as world}
            <div class="mb-6 pl-4">
              <div class="flex justify-between items-center mb-4">
                ...
                <a
                  href="/account/teams/{team.id}/worlds/{world.id}/elements/Event/create"
              ...
              {:else}
                ...
                >
                  <!-- Iterate over each event in the world and display it as a card -->
                  {#each world.elements as event}
                    <a
                      href={`/account/teams/${team.id}/worlds/${world.id}/elements/${event.id}`}
