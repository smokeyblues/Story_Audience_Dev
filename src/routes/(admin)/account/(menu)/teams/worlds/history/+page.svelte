<script lang="ts">
  ...
  $: teamsWithHistory = data.teamsWithHistory || []
</script>

<div class="p-4 md:p-8">
  <div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold">History & Lore</h1>
  </div>
  ...
  {#if teamsWithHistory.length === 0}
    <div class="text-center py-12">
      ...
      <p class="mt-2 text-neutral-content">
        You need to be a member of a team to create history & lore entries.
      </p>
      <a href="/account/teams" class="btn btn-primary mt-4">Go to Teams</a>
    </div>
  {:else}
    <!-- Iterate over each team -->
    {#each teamsWithHistory as team}
      <div class="mb-10">
          ...
          {#each team.worlds as world}
            <div class="mb-6 pl-4">
              <div class="flex justify-between items-center mb-4">
                ...
                <a
                  href="/account/teams/{team.id}/worlds/{world.id}/elements/Lore%20Entry/create"
              ...
              {#if world.elements.length === 0}
                <div class="bg-base-100 rounded-lg p-6 text-center">
                  <p class="text-neutral-content mb-3">
                    No history & lore entries in this world yet.
                  </p>
                  <a
                    href="/account/teams/{team.id}/worlds/{world.id}/elements/Lore%20Entry/create"
                    class="btn btn-sm btn-primary"
              ...
              {:else}
                ...
                >
                  <!-- Iterate over each history entry in the world and display it as a card -->
                  {#each world.elements as historyEntry}
                    <a
                      href={`/account/teams/${team.id}/worlds/${world.id}/elements/${historyEntry.id}`}
                      class="card card-compact bg-base-200 shadow-md hover:shadow-lg hover:-translate-y-1 transition-all duration-200"
                    >
                      <div class="card-body">
                        <h4 class="card-title">{historyEntry.name}</h4>
                        <p class="text-sm text-neutral-content truncate">
                          {(typeof historyEntry.properties === "object" &&
                            historyEntry.properties !== null &&
                            !Array.isArray(historyEntry.properties) &&
                            historyEntry.properties.text) ||
                            "No lore text available."}
                        </p>
                      </div>
                    </a>
                  {/each}
