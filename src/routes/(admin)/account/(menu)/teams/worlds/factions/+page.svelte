<script lang="ts">
  ...
  $: teamsWithFactions = data.teamsWithFactions || []
</script>

<div class="p-4 md:p-8">
  <div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold">All Factions</h1>
  </div>

  {#if teamsWithFactions.length === 0}
    <div class="text-center py-12">
      <p class="text-lg">No teams found.</p>
      <p class="mt-2 text-neutral-content">
        You need to be a member of a team to create factions.
      </p>
      <a href="/account/teams" class="btn btn-primary mt-4">Go to Teams</a>
    </div>
  {:else}
    <!-- Iterate over each team -->
    {#each teamsWithFactions as team}
      <div class="mb-10">
          ...
          {#each team.worlds as world}
            <div class="mb-6 pl-4">
              <div class="flex justify-between items-center mb-4">
                ...
                <a
                  href="/account/teams/{team.id}/worlds/{world.id}/elements/Faction/create"
              ...
              {:else}
                ...
                >
                  <!-- Iterate over each faction in the world and display it as a card -->
                  {#each world.elements as faction}
                    <a
                      href={`/account/teams/${team.id}/worlds/${world.id}/elements/${faction.id}`}
                      class="card card-compact bg-base-200 shadow-md hover:shadow-lg hover:-translate-y-1 transition-all duration-200"
                    >
                      <div class="card-body">
                        <h4 class="card-title">{faction.name}</h4>
                        <p class="text-sm text-neutral-content truncate">
                          {(typeof faction.properties === "object" &&
                            faction.properties !== null &&
                            !Array.isArray(faction.properties) &&
                            (faction.properties.description ||
                              faction.properties.goals)) ||
